...
#args for audio import
    ...
    INPUT_SectionName = "NPC_Barks"      #the section name, from BAT file. Use to find actor mixer and events parent for importing
    ...
    stepsUpToCommonDirectory = 3    # How many folders up from the script is the shared dir with wwise project

    DirOfWwiseProjectRoot = ["VrPark_HMD"] ## Name/path of wwise project relative to the common directory
...
###### MyComponent Importer Function Definitions #######
        ...
        def setupImportArgs(parentID, fileList,originalsPath):
            ...
            baseFileName = os.path.basename(audiofilename)

            fileSplit = baseFileName.split("_")
            print(fileSplit)

            lineFunction = fileSplit[6]
            gang = fileSplit[3]
            archetype = fileSplit[4]
            variation = fileSplit[5]
            RandContName = fileSplit[3]+"_"+fileSplit[4]+"_"+fileSplit[5]+"_"+fileSplit[6]
            ...
            # NPC barks should import into <Switch Container>
            barkLineFucntionSwitchContainer = "<Switch Container>"+lineFunction+"\\"

            NPCGangSwitch = "<Switch Container>"+gang+"\\"

            NPCArchetype = "<Switch Container>"+archetype+"\\"

            NPCActor = "<Switch Container>"+"Actor"+"\\"

            NPCRandomContainer = "<Random Container>"+RandContName+"\\"

            # Switch assignment

            importFilelist.append(
                {
                    "objectPath": sectionActorMixer + barkLineFucntionSwitchContainer + NPCGangSwitch + NPCArchetype+ NPCRandomContainer.replace("\\", ""),
                    "switchAssignation": "<Switch Group:NPC_Actor>"+variation
                }
            )

            importFilelist.append(
                {
                    "objectPath": sectionActorMixer + barkLineFucntionSwitchContainer + NPCGangSwitch + NPCArchetype.replace("\\",""),
                    "switchAssignation":"<Switch Group:NPC_Archetype>"+archetype
                }
            )
            importFilelist.append(
                {
                    "objectPath": sectionActorMixer + barkLineFucntionSwitchContainer + NPCGangSwitch.replace("\\", ""),
                    "switchAssignation": "<Switch Group:NPC_Gang>" + gang
                }
            )
            ...
            importFilelist.append(
                {
                    ...
                    "objectPath": sectionActorMixer + barkLineFucntionSwitchContainer + NPCGangSwitch + NPCArchetype+ NPCRandomContainer + objectType + baseFileName,
            ...
            eventPathContainer = barkLineFucntionSwitchContainer.replace("","")
            ActorMixerPath = "\\Actor-Mixer Hierarchy\\Greybox_dialogue_DEV\\Greybox_dialogue_DEV\\NPC_Barks\\"

            MyComponent.eventName = "VO_NPC_Bark_"+lineFunction
            MyComponent.eventTarget = ActorMixerPath+sectionActorMixer.replace("<Actor-Mixer>","")+ lineFunction
        ...
        def setupEventArgs(oactionType=1):
            ...
            MyComponent.createEventArgs = {
                ...
                "parent": "\\Events\\Greybox_Dialogue_DEV\\Greybox_Dialogue_Dev\\NPC_Barks",
                "type": "Event",
